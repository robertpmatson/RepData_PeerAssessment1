...
output: 
  html_document:
    keep_md: true
geometry: "top=0.5cm, bottom=0.5cm, left=2cm, right=1cm"
vignette: >
...
The median value of the number of steps is `r sprintf("%.3f", median(data$Steps)) `

The mean value of the number of steps is `r sprintf("%.3f", mean(data$Steps)) `

```{r, echo=TRUE, fig.show='hold', fig.width=9, fig.height=6}
...
The Interval with the highest average number of steps is `r slice(dataByInterval,1)$Interval `
...
Originally, total number of missing values is `r dim(activites[!complete.cases(activites),])[1] `
...
After the code above there are `r dim(activites[!complete.cases(activites),])[1]` missing values.

The new value of the median of the number of steps per day is: `r sprintf("%.3f", median(data$Steps)) `  
The new value of the mean of the number of steps per day is: `r sprintf("%.3f", mean(data$Steps)) `
